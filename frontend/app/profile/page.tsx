...
export default function ProfilePage() {
  ...
  const isDirty = nickname !== originalNickname || currentPw !== '' || newPw !== '';
  ...
  const handleChangePassword = async () => {
    if (!currentPw || !newPw) {
      toast.show('현재 비밀번호와 새 비밀번호를 모두 입력해주세요.', 'warning');
      return;
    }
    if (newPw.length < 8) {
  ...
  return (
    <main className="max-w-xl mx-auto px-6 py-12 space-y-10">
      ...
      <section className="space-y-2">
        <label className="block text-sm font-semibold text-clay-700 dark:text-cream-200">
          이메일
        </label>
      ...
      <section className="space-y-3">
        <label htmlFor="nickname" className="block text-sm font-semibold text-clay-700 dark:text-cream-200">
          닉네임
        </label>
        <input
          id="nickname"
