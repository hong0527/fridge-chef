...
2) 첫 문장: 사용자 문맥("{ctx}")을 '이 레시피'의 실제 종류(위 후보의 종류 표기)에 맞게 연결.
   ★중요: 이 레시피가 아닌 음식인 척 금지 — 메인요리인데 "뜨끈한 국물"처럼, 국물 아닌데 국물처럼
   쓰지 말 것. 종류가 문맥과 안 맞으면 억지로 그 종류로 묘사하지 말고, 이 레시피가 그 상황에
   어울리는 '다른 이유'(간편함·든든함·집밥 느낌 등)로 연결하라.
